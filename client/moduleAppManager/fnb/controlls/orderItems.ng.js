...
.controller("AppOrderItemsManagerCtrl", ["$scope", "$meteor", '$mdDialog', '$mdToast',
  function($scope, $meteor, $mdDialog, $mdToast) {
    ...
    $scope.page = 1;
    $scope.perPage = 5;
    $scope.sort = {createdAt: 1, orderId: 1};

    $scope.$meteorAutorun(function(){
      $scope.$meteorSubscribe('openOrders',{
        limit: parseInt($scope.getReactively('perPage')),
        skip: parseInt(($scope.getReactively('page') - 1) * $scope.getReactively('perPage')),
        sort: $scope.getReactively('sort')
      }, $scope.getReactively('searchKey'))
      .then(function(){
        $scope.ordersCount = $scope.$meteorObject(Counts, 'numberOfOpenOrderItems', false);
        if(!$scope.orders){
